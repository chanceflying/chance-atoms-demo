...
- **Cloudflare D1**：保存匿名 workspace 下的项目、当前记录和版本快照。
...
- workspace 由 `HttpOnly + SameSite=Lax` 匿名 Cookie 隔离。

这是笔试 Demo，不包含正式用户登录和团队权限。公网 Demo 不应直接挂载无限额模型密钥；生产化需要再加身份认证、Cloudflare Rate Limiting / Turnstile 和额度策略。
...
`smoke` 会验证首页、匿名 workspace 隔离、生成、项目持久化、空记录保存、迭代、服务端校验、回滚和独立 ZIP 导出，并清理测试项目。
