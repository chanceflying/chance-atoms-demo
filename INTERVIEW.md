...
- 两种能力的输入草稿互不共享；
- 首页包含示例输入和功能引导；
- 首页主按钮会随能力切换为“开始对话”或“开始构建”，使用完整文字而不是仅展示箭头；
- 首页输入框支持 `Enter` 提交、`Shift + Enter` 换行；
...
- Preview 自动缩放到桌面可视区域；
- 生成结果可导出为独立 ZIP；
- 构建详情输入框与首页保持一致，支持 `Enter` 发送、`Shift + Enter` 换行，并使用明确的“发送”按钮；
- 模型来源、版本摘要、构建说明和产物信息采用更清晰的字号与层级；
- 每次修改形成新的不可变版本；
- 历史版本可恢复为新的最新版本；
- 版本迭代完成消息会总结用户本次实际诉求；
...
- 对话详情使用统一输入提示和“发送”按钮，支持 `Enter` 发送、`Shift + Enter` 换行；
- 多标签页或多设备暂时没有服务端并发锁。
...
- Chat 项目、消息历史和失败保留；
- 用户显式控制的长期记忆；
- 首页与两类详情页统一输入提示、键盘快捷键和文字主按钮；
- 模型来源、构建摘要和产物信息的可读性优化；
- 自动标题、手动改名和删除；
