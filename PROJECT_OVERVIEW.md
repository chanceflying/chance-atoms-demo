...
- 首页、对话详情和 Web App 构建详情统一提示“输入消息，Enter 发送，Shift + Enter 换行…”；
- 三个输入区域都实际支持 `Enter` 提交、`Shift + Enter` 换行；
- 首页使用“开始对话 / 开始构建”文字主按钮，详情页统一使用“发送”，避免只有箭头时产生理解成本；
- 模型来源、版本摘要、构建说明和产物信息提高字号并统一层级，确保演示过程中能够快速读懂当前状态。
...
| 状态 | 内容 |
| --- | --- |
| 已完成 | 方案生成与调整、Web App 生成、Preview、代码查看、导出、版本演进与恢复、持续对话、长期记忆、统一输入快捷键与文字主按钮、项目管理、GitHub 登录、D1 持久化和 Cloudflare 部署 |
